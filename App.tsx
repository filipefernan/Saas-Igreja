...
const AppContainer = () => {
    ...
    useEffect(() => {
        // AppContext's useEffect will set the initial state from localStorage.
        // We just give it a moment to load to prevent UI flicker.
        setTimeout(() => setStateLoaded(true), 50);
    }, []);
    ...
    if (!context) {
        // This can happen briefly on the first render before context is available.
        return <div className="w-full h-screen bg-[var(--color-background-secondary)]"></div>;
    }
    ...
    return (
        <>
            <div className="flex h-screen bg-[var(--color-background-secondary)] overflow-hidden">
                <Sidebar activePage={activePage} setActivePage={navigateTo} openChat={() => setTestChatOpen(true)} />
